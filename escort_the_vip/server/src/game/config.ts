...
export interface GameConfig {
  ...
  // Damage multipliers per blast radius (applied to the random roll).
  // Normal explosion uses R1/R2; chain explosion uses CHAIN_R1/R2/R3.
  BOMB_DMG_MULT_R1: number        // normal  radius 1 (default 1.0 = full damage)
  BOMB_DMG_MULT_R2: number        // normal  radius 2 (default 0.5 = half damage)
  BOMB_CHAIN_R1_CHANCE: number    // probability [0..1] of chain-trigger at radius 1 (clear path)
  BOMB_CHAIN_R2_CHANCE: number    // probability [0..1] of chain-trigger at radius 2 (clear path)
  // Chain through an obstacle at R1 (blocked path):
  BOMB_CHAIN_BLOCKED_CONCRETE_CHANCE: number  // through WALL_CONCRETE (default 0.125)
  BOMB_CHAIN_BLOCKED_OTHER_CHANCE:    number  // through any other wall type (default 0.25)
  BOMB_DMG_CHAIN_MULT_R1: number  // chain blast radius 1 multiplier
  BOMB_DMG_CHAIN_MULT_R2: number  // chain blast radius 2 multiplier
  BOMB_DMG_CHAIN_MULT_R3: number  // chain blast radius 3 multiplier (chain-only)
  BOMB_CHAIN_CONCRETE_DESTROY_CHANCE: number  // [0..1] prob. of destroying a WALL_CONCRETE at R1 on chain blast
...
function validate(raw: any): GameConfig {
  ...
  const prob = (k: string): number => {
    const v = num(k)
    if (v < 0 || v > 1) throw new Error(`${k}: must be in [0, 1]`)
    return v
  }
  ...
  return {
    ...
    BOMB_DMG_MULT_R1:        pos ('BOMB_DMG_MULT_R1'),
    BOMB_DMG_MULT_R2:        pos ('BOMB_DMG_MULT_R2'),
    BOMB_CHAIN_R1_CHANCE:              prob('BOMB_CHAIN_R1_CHANCE'),
    BOMB_CHAIN_R2_CHANCE:              prob('BOMB_CHAIN_R2_CHANCE'),
    BOMB_CHAIN_BLOCKED_CONCRETE_CHANCE: prob('BOMB_CHAIN_BLOCKED_CONCRETE_CHANCE'),
    BOMB_CHAIN_BLOCKED_OTHER_CHANCE:    prob('BOMB_CHAIN_BLOCKED_OTHER_CHANCE'),
    BOMB_DMG_CHAIN_MULT_R1:  pos ('BOMB_DMG_CHAIN_MULT_R1'),
    BOMB_DMG_CHAIN_MULT_R2:  pos ('BOMB_DMG_CHAIN_MULT_R2'),
    BOMB_DMG_CHAIN_MULT_R3:              pos ('BOMB_DMG_CHAIN_MULT_R3'),
    BOMB_CHAIN_CONCRETE_DESTROY_CHANCE: prob('BOMB_CHAIN_CONCRETE_DESTROY_CHANCE'),
...
try {
  ...
  console.error(`[Config] fs.watch unavailable — hot-reload disabled:`, (e as Error).message)
}
