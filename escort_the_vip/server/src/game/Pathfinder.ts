import {
  ...
  WALL_NONE, WALL_METAL_DOOR, WALL_WOOD_DOOR, WALL_WOOD_FENCE, WALL_HEDGE,
...
// Destructible solids (wood fence, hedge): passable only after destruction.
// Indestructible solids (concrete, metal wall): never passable.
//
...
export function isPassable(type: number, state: number): boolean {
  ...
  if (type === WALL_METAL_DOOR || type === WALL_WOOD_DOOR) {
    return state === STATE_OPEN || state === STATE_DESTROYED
  }
  if (type === WALL_WOOD_FENCE || type === WALL_HEDGE) {
    return state === STATE_DESTROYED
  }
  return false  // concrete, metal wall — always solid
}
