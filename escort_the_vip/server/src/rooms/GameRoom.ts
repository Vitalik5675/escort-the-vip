...
import {
  GRID_COLS, GRID_ROWS, H_WALL_COUNT, hWallIndex, vWallIndex, PLAYER_MAX_HP, VIP_MAX_HP,
  ATTACK_DAMAGE_MIN, ATTACK_DAMAGE_MAX, BATON_BONUS_MIN, BATON_BONUS_MAX,
  BOMB_DAMAGE_MIN, BOMB_DAMAGE_MAX, BOMB_SELF_DAMAGE,
  ATTACK_COOLDOWN_MS,
  GAME_DURATION_S, COUNTDOWN_S, END_LINGER_S, MIN_PLAYERS_START, MAX_PLAYERS,
  VIP_ROOM_LOCK_S,
  ITEM_SPAWN_INTERVAL_MS, ITEM_MAX, ITEM_INITIAL_COUNT, BATON_MAX_TOTAL, ITEM_SPAWN_WEIGHTS,
  NPC_MOVE_INTERVAL_MS, VIP_BYPASS_MS, VIP_FLEE_TILES, WALL_NONE, WALL_METAL_DOOR, WALL_WOOD_DOOR, WALL_WOOD_FENCE, WALL_HEDGE,
  STATE_SOLID, STATE_OPEN, STATE_DESTROYED, STATE_BLOCKED,
  DISCONNECT_DEATH_MS, DISCONNECT_PURGE_MS, WALL_COUNT,
  WALL_DMG_HAND_MIN, WALL_DMG_HAND_MAX,
  WALL_DMG_BATON_MIN, WALL_DMG_BATON_MAX,
  WALL_DMG_BOMB_MIN, WALL_DMG_BOMB_MAX,
  SHIELD_MAX_HP_MIN, SHIELD_MAX_HP_MAX,
  SHIELD_HIT_BY_HAND, SHIELD_HIT_BY_BATON, SHIELD_HIT_BY_BOMB,
  SHIELD_MAX_TOTAL,
  ATTACK_RESOLVE_DELAY_MS, DEFENDING_DAMAGE_MULT, BOMB_FUSE_MS,
  EMPTY_ARENA_MODE,
  randInt,
} from '../game/constants'
...
export class GameRoom extends Room<{ state: GameState }> {
  ...
  // Timestamp (Date.now()) when VIP started trying to bypass a blocking hater.
  // 0 = no active bypass attempt. Once VIP_BYPASS_MS elapses without success
  // the VIP gives up following and switches to full flee mode.
  private vipBypassStart       = 0
  // VIP's tile at the moment a hater last became adjacent (updated on every
  // new hater encounter, including during an ongoing flee). VIP must reach
  // VIP_FLEE_TILES Manhattan tiles from this origin before she may stop.
  private vipFleeOrigin: { col: number; row: number } | null = null

  // Phase 5 — pending-attack queue. Holds each attack for ATTACK_RESOLVE_DELAY_MS
  ...
  // EMPTY_ARENA_MODE — persistent demo item tiles.
  ...
  onJoin(client: Client, _options: Record<string, unknown> | undefined, auth: AuthPayload) {
    ...
    if (restored) {
      ...
      if (restored.isAlive && restored.zone === 'game') {
        ...
        client.send('teleport', { x: 16, y: 1.1, z: 14.9, cx: 16, cy: 1.5, cz: 13.75, spectating: true })
  ...
  private setupMessages() {
    ...
    this.onMessage('cancel_join', (client: Client) => {
      ...
      p.inQueue = false
      this.state.queueCount--
      if (this.state.phase === 'countdown' && this.state.queueCount < MIN_PLAYERS_START) {
    ...
    this.onMessage('player_tile', (client: Client, data: { col: number; row: number }) => {
      ...
      if (this.isTileOccupiedByOther(newCol, newRow, client.sessionId, allowVipOverlap)) { teleportBack(); return }
  ...
  private gameLoop(dt: number) {
    ...
    if (phase === 'playing') {
      ...
      if (!EMPTY_ARENA_MODE) { this.itemTimer += dt; if (this.itemTimer >= ITEM_SPAWN_INTERVAL_MS) { this.itemTimer = 0; this.spawnItem() } }
    ...
    if (phase === 'ended') {
      this.endTimer += dt
      if (this.endTimer >= END_LINGER_S * 1000) this.resetToLobby()
    }
  }

  private npcMoveInterval(): number {
    const vip = this.state.vip
    if (!vip.active || vip.maxHealth === 0) return NPC_MOVE_INTERVAL_MS
    const hpFrac   = vip.health / vip.maxHealth
    const timeFrac = GAME_DURATION_S > 0 ? this.state.timeRemaining / GAME_DURATION_S : 0
    const urgency  = 1 - Math.min(hpFrac, timeFrac)
    // Floor at 1000ms per tile so the VIP never moves faster than a walking
    // player (GAME_WALK_SPEED 2 m/s · 2 m tile = exactly 1 s/tile). Speeding up
    // beyond that would let the VIP escape unprotected and trivialise haters.
    const PLAYER_TILE_MS = 1000
    return Math.max(PLAYER_TILE_MS, NPC_MOVE_INTERVAL_MS / (1 + urgency * 3))
  }

  // ── Phase transitions ────────────────────────────────────────────────────

  private checkCountdown() {
    if (this.state.phase === 'lobby' && this.state.queueCount >= MIN_PLAYERS_START) {
  ...
  private prepareMazeForMatch() {
    ...
    // In EMPTY_ARENA_MODE the maze is replaced with a flat open field that
    ...
    this.applyMazeConfig(EMPTY_ARENA_MODE ? generateEmptyArena() : generateRandomMaze())

    const vip = this.state.vip
    vip.health      = VIP_MAX_HP
    vip.maxHealth   = VIP_MAX_HP
  ...
  private startGame() {
    ...
    this.state.timeRemaining = GAME_DURATION_S
    ...
    for (const [sid, p] of this.state.players.entries()) {
      ...
      p.zone = 'game'; p.health = PLAYER_MAX_HP; p.maxHealth = PLAYER_MAX_HP
    ...
    // In EMPTY_ARENA_MODE, place one permanent demo item of each type in a row
    ...
    // In EMPTY_ARENA_MODE the three fixed demo items are enough — skip random spawns.
    if (!EMPTY_ARENA_MODE) {
      for (let i = 0; i < ITEM_INITIAL_COUNT; i++) this.spawnItem()
    }
  ...
  private endGame(winner: string, reason: string) {
    ...
    // lingering on the maze through the END_LINGER_S countdown.
    ...
    result.durationS = Math.max(0, Math.round(GAME_DURATION_S - this.state.timeRemaining))
    ...
    for (const [sid, p] of this.state.players.entries()) {
      if (p.zone !== 'lobby') {
        p.zone = 'lobby'; p.team = 'none'
        this.clients.find(c => c.sessionId === sid)?.send('teleport', { x: 8, y: 1.1, z: 4, cx: 16, cy: 1.1, cz: 16, spectating: false })
      }
    }
  ...
  private unlockOneVipRoomDoor() {
    ...
    console.log(`[GameRoom] VIP-room door ${pick} unblocked (closed, ready to open)`)
  }
  ...
  private moveNpc() {
    ...
    if ((this.vipFleeing || this.vipBypassStart > 0) && adjBGEntry) {
      this.vipFleeing     = false
      this.vipFleeLastDir = null
      this.vipBypassStart = 0
      this.vipFleeOrigin  = null
      // VIP exits flee/bypass when a bodyguard is adjacent — she stops running
      // but does NOT automatically start following that bodyguard. followerId is
      // only set when a bodyguard explicitly clicks VIP (handleFollowVip).
      // If followerId is already set to a live BG it stays — VIP resumes escort.
    }

    // 2. Hater adjacent.
    const adjHater = this.findAdjacentHater(vip.tileCol, vip.tileRow)
    if (adjHater) {
      ...
      // No adjacent BG — record escape origin and enter bypass/flee.
      // Record (or refresh) the escape origin every time a hater is adjacent.
      // Step 4 uses this position to count how many tiles VIP has put between
      // herself and the last encounter before allowing her to stop.
      this.vipFleeOrigin = { col: vip.tileCol, row: vip.tileRow }

      if (!this.vipFleeing) {
        // ── Bypass phase ──────────────────────────────────────────────────
        // VIP tries to walk around the hater and reach the bodyguard through
        // any alternate route. Hater tiles are treated as impassable so the
        // path is guaranteed to avoid them. If no route exists at this tick
        // she simply waits — the timer keeps running.
        const follower = vip.followerId ? this.state.players.get(vip.followerId) : null

        if (this.vipBypassStart === 0) this.vipBypassStart = now

        const bypassExpired = now - this.vipBypassStart >= VIP_BYPASS_MS

        if (!bypassExpired && follower && follower.isAlive && follower.zone === 'game') {
          const fleeStates = this.vipFleePassableStates(wt, ws)
          const haterTiles = this.haterTileSet()
          const path = findPath(
          ...
          // No bypass route yet — hold position for this tick.
          return
        }

        // Grace period expired — VIP cannot bypass the hater to reach the
        // bodyguard. Enter flee mode but KEEP followerId/targetName: after
        // fleeing VIP_FLEE_TILES away the flee phase ends and step 5 (escort)
        // resumes toward the same bodyguard automatically.
        // followerId is only cleared by: bodyguard death (refreshVipFollower),
        // another bodyguard's explicit click (handleFollowVip), or
        // game end / lobby reset.
        this.vipBypassStart = 0
      }
    ...
    // 3. No adjacent hater — clear bypass timer (hater moved away or was bypassed).
    this.vipBypassStart = 0

    // 4. Flee mode: VIP actively seeks the tile maximally far from all haters,
    //    routing through closed doors. She stops once she is VIP_FLEE_TILES
    ...
    if (this.vipFleeing) {
      const tilesAway = this.vipFleeOrigin
        ...
        : VIP_FLEE_TILES   // no origin → treat as done
      const fleeDone = tilesAway >= VIP_FLEE_TILES
      ...
      const remainingTiles = VIP_FLEE_TILES - tilesAway
      const fleeTarget = this.pickVipFleeDest(vip, wt, ws, false)
      if (fleeTarget) {
        const fleeStates = this.vipFleePassableStates(wt, ws)
        const haterTiles = this.haterTileSet()
        ...
        if (path.length >= 2) {
          ...
          const crossesDoor  = crossWall >= 0
            && (wt[crossWall] === WALL_METAL_DOOR || wt[crossWall] === WALL_WOOD_DOOR)
    ...
    if (manh <= 1) {
      ...
      if (canMoveBetween(vip.tileCol, vip.tileRow, f.tileCol, f.tileRow, wt, ws)) {
        ...
        const threatHater =
          this.findAdjacentHater(vip.tileCol, vip.tileRow) ??
          this.findAdjacentHater(f.tileCol,   f.tileRow)   ??
  ...
  private stepVipNormal(vip: VIPState, next: { col: number; row: number }) {
    ...
    if (this.safetyTileSet.has(vip.tileRow * GRID_COLS + vip.tileCol)) {
      vip.reachedSafe = true; this.endGame('bodyguards', 'vip_safe')
    }
  }
  ...
  private isDoorAdjacent(col: number, row: number, wt: number[]): boolean {
    const isDoor = (idx: number) => {
      ...
      return wt[idx] === WALL_METAL_DOOR || wt[idx] === WALL_WOOD_DOOR
    }
  ...
  private pickVipFleeDest(
    ...
    for (let r = 0; r < GRID_ROWS; r++) {
      for (let c = 0; c < GRID_COLS; c++) {
        ...
        const bonus = doorBonusTiles.has(r * GRID_COLS + c) ? DOOR_BONUS : 0
        candidates.push({ col: c, row: r, score: hDist + bonus })
      }
    }
  ...
  private findAdjacentHater(col: number, row: number): PlayerState | null {
    for (const p of this.state.players.values()) {
      if (p.team !== 'hater' || p.zone !== 'game' || !p.isAlive) continue
      // Chebyshev ≤ 1: all 8 neighbours (cardinal + diagonal). Haters can hit
      // diagonally (same attack-range check as client-side isLocalNearTileDiag),
      // so VIP must react to diagonal threats too.
      const dc = Math.abs(p.tileCol - col), dr = Math.abs(p.tileRow - row)
      if (dc <= 1 && dr <= 1 && (dc + dr) > 0) return p
    }
    return null
  }
  ...
  private vipFleePassableStates(wt: number[], ws: number[]): number[] {
    ...
    for (let i = 0; i < wt.length; i++) {
      const t = wt[i]
      if (t !== WALL_METAL_DOOR && t !== WALL_WOOD_DOOR) continue
  ...
  // Cell-adjacency: returns wall indices that are DOORS (metal / wood) and
  // sit at the 4 cardinal edges of the given tile.
  private adjacentDoorsTo(col: number, row: number): number[] {
    ...
    const check = (idx: number) => {
      ...
      if (t === WALL_METAL_DOOR || t === WALL_WOOD_DOOR) out.push(idx)
    }
  ...
  // Phase 5: enqueue attack; defending players are barred from attacking.
  private handleAttack(attackerId: string, targetId: string) {
    if (this.state.phase !== 'playing') return
    const a = this.state.players.get(attackerId)
    if (!a || !a.isAlive || a.zone !== 'game' || a.defending) return
    ...
    const now = Date.now()
    if ((this.attackCooldowns.get(attackerId) ?? 0) + ATTACK_COOLDOWN_MS > now) return
    ...
    this.pendingAttacks.set(attackerId, {
      ...
      isVip,
      src,
      scheduledAt: now + ATTACK_RESOLVE_DELAY_MS,
    })
  }
  ...
  private resolveAttack(attackerId: string, info: PendingAttack, skipKnockback: boolean) {
    ...
    const baseDmg = randInt(ATTACK_DAMAGE_MIN, ATTACK_DAMAGE_MAX) +
                    (info.src === 'baton' ? randInt(BATON_BONUS_MIN, BATON_BONUS_MAX) : 0)
    ...
    if (info.isVip) {
      ...
      if (vip.health <= 0) this.endGame('haters', 'vip_killed')
      return
    }
    ...
    const result = this.applyDamageToPlayer(t, baseDmg, info.src, a.rightHand === 'baton')
  ...
  private applyDamageToPlayer(target: PlayerState, baseDmg: number, src: 'hand'|'baton'|'bomb', attackerHasBaton: boolean): { hpDmg: number; shieldDmg: number; fullyBlocked: boolean } {
    const hasShield = target.leftHand === 'shield' && target.shieldHP > 0
    const shieldHit = src === 'hand' ? SHIELD_HIT_BY_HAND : src === 'baton' ? SHIELD_HIT_BY_BATON : SHIELD_HIT_BY_BOMB

    // Full-block: defending + shield + attacker has no baton.
    if (target.defending && hasShield && !attackerHasBaton) {
      const shieldDmg = Math.min(target.shieldHP, shieldHit)
      this.deductShield(target, shieldDmg)
      return { hpDmg: 0, shieldDmg, fullyBlocked: true }
    }

    // Defending: HP damage halved; shield (if any) also takes its hit.
    if (target.defending) {
      const hpDmg = Math.max(1, Math.floor(baseDmg * DEFENDING_DAMAGE_MULT))
      target.health = Math.max(0, target.health - hpDmg)
      target.lastDamage = hpDmg; target.lastDamageTime = Date.now()
      let shieldDmg = 0
      if (hasShield) {
        shieldDmg = Math.min(target.shieldHP, shieldHit)
        this.deductShield(target, shieldDmg)
      }
      return { hpDmg, shieldDmg, fullyBlocked: false }
    }

    // Not defending + shield: damage split half-half (HP / shield).
    if (hasShield) {
    ...
    // Not defending, no shield: full HP damage.
  ...
  // BOMB_FUSE_MS countdown starts immediately.
  private handlePlaceBomb(sessionId: string, wallIdx: number) {
    ...
    if (wallIdx >= 0 && wallIdx < this.state.wallTypes.length) {
      ...
      const isDoor = wt === WALL_METAL_DOOR || wt === WALL_WOOD_DOOR
      // Any door (metal or wood, regardless of BLOCKED/SOLID state) that is
      // adjacent to the player's tile becomes a door-trap, never a countdown
      // bomb. The BLOCKED exception was removed because autoClaimAdjacentDoors
    ...
    bomb.fuseEndsAt     = trapWallIdx < 0 ? now + BOMB_FUSE_MS : 0
  ...
  private detonateBomb(key: string, bomb: BombState) {
    ...
    const rollFull = () => randInt(BOMB_DAMAGE_MIN, BOMB_DAMAGE_MAX)
    ...
    for (const [sid, p] of this.state.players.entries()) {
      ...
      if (cheb === 0) {
        const dmg = sid === ownerId ? BOMB_SELF_DAMAGE : rollFull()
      ...
      const baseDmg = cheb === 1
        ? (sid === ownerId ? BOMB_SELF_DAMAGE : rollFull())
        : halve(sid === ownerId ? BOMB_SELF_DAMAGE : rollFull())
  ...
  private applyBombHitToPlayer(
    ...
    let killed = false
    if (baseDmg > 0) {
      const result = this.applyDamageToPlayer(p, baseDmg, 'bomb', false)
  ...
  private handlePlayerDeath(sessionId: string, p: PlayerState, killerId: string): void {
    ...
    client.send('teleport', {
      x: 16,  y: 1.1, z: 14.9,
      cx: 16, cy: 1.5, cz: 13.75,
      spectating: true
    })
  }

  private checkWinByElimination() {
    const alive = [...this.state.players.values()].filter(p => p.zone === 'game' && p.isAlive)
  ...
  private handleWallAttack(sessionId: string, wallIdx: number) {
    ...
    // a destructible wall.
    const now = Date.now()
    if ((this.attackCooldowns.get(sessionId) ?? 0) + ATTACK_COOLDOWN_MS > now) return
  ...
  private attackBlockReason(type: number, hp: number, maxHp: number, src: 'hand' | 'baton' | 'bomb'): string {
    ...
    return ''   // wood fence accepts everything; nothing else is destructible
  }
  ...
  private damageWall(idx: number, src: 'hand' | 'baton' | 'bomb'): boolean {
    ...
    if (maxHp <= 0) return false   // indestructible (concrete / iron / metal door / NONE)
    if (hp <= 0)    return false

    let allowed = false
    if (type === WALL_WOOD_FENCE) {
    ...
    const dmg = src === 'hand'
      ? randInt(WALL_DMG_HAND_MIN,  WALL_DMG_HAND_MAX)
      : src === 'baton'
        ? randInt(WALL_DMG_BATON_MIN, WALL_DMG_BATON_MAX)
        : randInt(WALL_DMG_BOMB_MIN,  WALL_DMG_BOMB_MAX)
  ...
  private handleDoor(sessionId: string, wallIdx: number) {
    if (wallIdx < 0 || wallIdx >= this.state.wallTypes.length) return
    const type = this.state.wallTypes[wallIdx]
    if (type !== WALL_METAL_DOOR && type !== WALL_WOOD_DOOR) return
    const p = this.state.players.get(sessionId)
    if (!p || p.zone !== 'game' || !p.isAlive) return
  ...
  private spawnItem() {
    if (this.state.items.size >= ITEM_MAX) return
    if (!this.mazeConfig) return
    const pool  = this.mazeConfig.spawnPool
    if (pool.length === 0) return

    // Pick a type via weighted RNG (bombs spawn ~3× as often as baton/shield).
    // If the rolled type is "baton" and we're already at BATON_MAX_TOTAL, drop
    ...
    const weights: Record<string, number> = { ...ITEM_SPAWN_WEIGHTS }
    if (batonCount  >= BATON_MAX_TOTAL)  delete weights.baton
    if (shieldCount >= SHIELD_MAX_TOTAL) delete weights.shield
  ...
  // BATON_MAX_TOTAL across the whole game, not just floor items.
  ...
  private handlePickup(sessionId: string, key: string) {
    ...
    if (item.type === 'baton') {
      ...
      const rolledMaxHp = randInt(SHIELD_MAX_HP_MIN, SHIELD_MAX_HP_MAX)
    ...
    // In EMPTY_ARENA_MODE, demo item tiles are never left empty — re-spawn
  ...
  private isTileOccupiedByOther(col: number, row: number, ignoreSessionId: string, allowVip = false): boolean {
    for (const [sid, p] of this.state.players.entries()) {
      if (sid === ignoreSessionId) continue
      if (!p.connected) continue
      if (p.zone === 'game' && p.tileCol === col && p.tileRow === row) return true
    }
  ...
  private processDisconnectGrace() {
    ...
    for (const [sid, p] of [...this.state.players.entries()]) {
      ...
      if (elapsed >= DISCONNECT_PURGE_MS) {
        if (p.inQueue) this.state.queueCount--
      ...
      } else if (p.isAlive && elapsed >= DISCONNECT_DEATH_MS && p.zone === 'game') {
        p.isAlive   = false
        p.killerId  = ''
