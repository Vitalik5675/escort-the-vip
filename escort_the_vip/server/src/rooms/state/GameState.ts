...
export class PlayerState extends Schema {
  ...
  // Damage *dealt* by this player on their most recent resolved attack.
  // Replicated so spectators can show the watched attacker's damage HUD.
  @type('number')  lastDealt:       number  = 0
  @type('number')  lastDealtTime:   number  = 0
...
export class VIPState extends Schema {
  ...
  // Facing direction in DCL yaw degrees (around Y axis).
  // 0 = facing north (-Z), 90 = east (+X), 180 = south (+Z), -90 = west (-X).
  // Updated every tick by the server (movement direction, or nearest adjacent player).
  @type('number')  facingYaw:      number  = 0
  ...
  // Appearance config — loaded from vipConfig in server/src/game/constants.ts at startup.
  //   'random'  — random male/female + random outfit indices (default)
  //   'female'  — always female body, random outfit indices
  //   'male'    — always male body, random outfit indices
  //   'player'  — mirror a specific wallet's avatar (server fetches from Catalyst)
  @type('string') appearanceMode: string = 'random'
  // Informational: wallet address used when appearanceMode === 'player'.
  @type('string') playerWallet:   string = ''
  // Serialised avatar data for 'player' mode.  Non-empty string means
  // the server successfully fetched the Catalyst profile and applied any
  // overrides from vipConfig.overrides (game/constants.ts).  Format (JSON):
  //   { bodyShape, wearables, skinColor:{r,g,b}, hairColor:{r,g,b}, eyeColor:{r,g,b} }
  // Empty string → client falls back to the indices-based base avatar.
  @type('string') playerAvatarJson: string = ''
}
