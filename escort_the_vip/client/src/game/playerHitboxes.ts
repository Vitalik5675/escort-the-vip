...
import { createEntity, makeClickable, removeEntity, setEKeyHint, clearEKeyHint } from '../utils/entityFactory'
...
export function upsertEnemyHitbox(
  ...
  const localTeam = s.myTeam
  const isEnemy   = localTeam !== 'none' && team !== 'none' && team !== localTeam

  // ALLOW_ALL_TILE_OVERLAP collapses every player to "ally" — no hitbox at all
  // (no physics block + no click). Picked up live from scene_config; the
  // syncAllEnemyHitboxesForOverlap() helper below tears existing hitboxes down
  // when the flag flips so this branch decides the next state cleanly.
  if (!isEnemy || !alive || s.allowAllTileOverlap) {
  ...
  if (!hb) {
    ...
    makeClickable(clickBox, `Attack ${displayName || 'Player'}`, () => {
      ...
      // Gate with client-side cooldown that mirrors server ATTACK_COOLDOWN_MS.
      // Prevents rapid clicks from being silently dropped server-side with
      // no feedback — the player sees a brief toast instead.
      if (!tryGateAttack()) return
      sendToRoom('attack', { targetId: sessionId })
    }, 3.5)

    // Bomb-throw hint: shown alongside the attack label when player holds a bomb.
    // refreshEnemyBombHints() keeps this in sync whenever inventory changes.
    if (getLocalState().myLeftHand === 'bomb') {
      setEKeyHint(clickBox, 'Throw Bomb', 3.5, () => {
        ...
        sendToRoom('throw_bomb', { targetId: sessionId })
      })
    }
  ...
  if (displayName && displayName !== hb.name) {
    ...
    // Re-apply bomb hint after makeClickable (it rebuilds PointerEvents).
    if (getLocalState().myLeftHand === 'bomb') {
      setEKeyHint(hb.clickBox, 'Throw Bomb', 3.5, () => {
        const ls = getLocalState()
        if (ls.myLeftHand !== 'bomb') return
        const target = hitboxes.get(sessionId)
        if (!target) return
...
/**
 * Drop all enemy hitboxes when ALLOW_ALL_TILE_OVERLAP flips. The next state
 * sync re-calls upsertEnemyHitbox for every visible player, which will then
 * see the live flag and either skip creation (overlap on) or rebuild as
 * usual (overlap off).
 */
export function syncAllEnemyHitboxesForOverlap() {
  for (const sid of [...hitboxes.keys()]) removeEnemyHitbox(sid)
}

/**
 * Show or hide the "Throw Bomb [E]" hint on every enemy click cylinder.
 * Call this whenever myLeftHand changes (same sites as refreshWallEKeyLabels).
 */
export function refreshEnemyBombHints(hasBomb: boolean): void {
  for (const [sid, hb] of hitboxes.entries()) {
    if (hasBomb) {
      setEKeyHint(hb.clickBox, 'Throw Bomb', 3.5, () => {
        const ls = getLocalState()
        if (ls.myLeftHand !== 'bomb') return
        const target = hitboxes.get(sid)
        if (!target) return
        if (!isLocalNearTileDiag(target.col, target.row)) return
        consumeWallEKey()   // suppress global place_bomb handler
        sendToRoom('throw_bomb', { targetId: sid })
      })
    } else {
      clearEKeyHint(hb.clickBox)
    }
  }
}
