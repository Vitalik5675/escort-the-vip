...
import { isLocalAdjacentToWall, isLocalNearTile } from '../utils/adjacency'
...
// ── Pickup range helpers ──────────────────────────────────────────────────────

/**
 * Wall index between two cardinal-adjacent tiles. Mirrors server Pathfinder's
 * wallIdxBetween — kept local so maze.ts stays independent of server code.
 * Returns -1 if the tiles are not cardinal neighbours.
 */
function wallIdxBetweenTiles(
  playerCol: number, playerRow: number,
  itemCol:   number, itemRow:   number,
): number {
  const dc = itemCol - playerCol
  const dr = itemRow - playerRow
  if      (dc === 0 && dr ===  1) return hWallIndex(playerRow, playerCol)
  else if (dc === 0 && dr === -1) return hWallIndex(itemRow,   playerCol)
  else if (dc === 1 && dr ===  0) return vWallIndex(playerRow, playerCol)
  else if (dc === -1 && dr === 0) return vWallIndex(playerRow, itemCol)
  return -1
}

/**
 * True if the player at (playerCol, playerRow) may pick up an item at
 * (itemCol, itemRow).
 *
 * Rules (mirror of server canPickupBetweenTiles):
 *   • Same tile              → always OK.
 *   • Cardinal neighbour     → OK when the wall slot between them has no live
 *     wall entity. wallEntities is keyed on wallIdx and only contains walls
 *     that are NOT destroyed (shouldExist = type≠NONE && state≠DESTROYED), so:
 *       – WALL_NONE        → never inserted    → pickup OK ✓
 *       – STATE_DESTROYED  → removed on sync   → pickup OK ✓
 *       – STATE_OPEN door  → entry kept (open door still blocks pickup) ✓
 *   • Any other distance     → not OK.
 */
export function isPickupInRange(
  playerCol: number, playerRow: number,
  itemCol:   number, itemRow:   number,
): boolean {
  if (playerCol === itemCol && playerRow === itemRow) return true
  if (Math.abs(playerCol - itemCol) + Math.abs(playerRow - itemRow) !== 1) return false
  const wallIdx = wallIdxBetweenTiles(playerCol, playerRow, itemCol, itemRow)
  if (wallIdx < 0) return true   // tiles are at the grid boundary — no wall slot
  // wallEntities has an entry for every wall that exists and is NOT destroyed.
  // WALL_NONE tiles are never inserted, so absence ↔ free passage.
  return !wallEntities.has(wallIdx)
}
...
// Hash of the last buildMaze() call (wallTypes + wallStates joined).
// Used to skip a full entity rebuild when the same layout arrives twice
// (e.g. rebuildFromState + maze_rebuild in the same frame on join).
let _lastBuildMazeHash = ''
...
// ── Spectator wall height ─────────────────────────────────────────────────────
// When the local player enters spectator mode the maze walls and junction posts
// are scaled down from 2.5 m to 2.0 m so the top-down VirtualCamera has a
// clearer view of the arena. Restored to full height when spectating ends.
//
// Because jumping is disabled in the game zone the 2 m physics collider still
// fully blocks all players — gameplay is unaffected.
//
// Implementation: single pass over wallEntities + junctionEntities, mutating
// Transform.scale.y and Transform.position.y in-place (no new entities, no
// Tweens — instant swap at the moment the VirtualCamera activates / deactivates).

const WALL_SPECTATOR_HEIGHT = 2.0
let _spectatorWallScaleActive = false

export function applySpectatorWallScale(active: boolean): void {
  if (_spectatorWallScaleActive === active) return
  _spectatorWallScaleActive = active

  const height  = active ? WALL_SPECTATOR_HEIGHT : WALL_HEIGHT
  const centerY = GAME_ZONE_Y + height / 2

  for (const w of wallEntities.values()) {
    const t = Transform.getMutableOrNull(w.entity)
    if (!t) continue
    t.scale.y = height
    // Skip open doors — their position is managed by the slide animation.
    // Update finalY so the next close-animation targets the right height.
    if (w.state !== STATE_OPEN) {
      t.position.y = centerY
    }
    w.finalY = centerY
  }

  for (const e of junctionEntities.values()) {
...
function makeWallEKeyCallback(wallIdx: number): (entity: Entity) => void {
  return (_entity: Entity) => {
    ...
    // Client-side cooldown gate: mirrors server ATTACK_COOLDOWN_MS so rapid
    // E-key presses don't fire invisible rejected attacks. Wall bomb placement
    // is exempt (no cooldown on the server for place_bomb).
...
export function refreshWallEKeyLabels(rightHand: string, leftHand: string): void {
  ...
  for (const [wallIdx, w] of wallEntities.entries()) {
    // Open doors have no MeshCollider — skip them entirely so we don't
    // pollute the eKeyRegistry with hints that can never fire.
    // syncPointerEvents() also guards against this, but skipping here avoids
    // unnecessary registry churn on every inventory change.
    if (w.state === STATE_OPEN) continue
...
/** Fast djb2-style hash of two parallel integer arrays (same length). */
function _hashWalls(types: number[], states: number[]): number {
  let h = 5381
  for (let i = 0; i < types.length; i++) {
    // djb2: h = ((h << 5) + h) ^ value  — works in 32-bit signed JS integers
    h = (Math.imul(h, 33) ^ types[i])  | 0
    h = (Math.imul(h, 33) ^ states[i]) | 0
  }
  return h
}

// ── DCL entity ID versioning — why the IDs look large ────────────────────────
//
// DCL entity IDs are 32-bit: [version(16 bits) | entityNumber(16 bits)]
// e.g. entity ID 132,000 = entityNumber 928, version 2 = the THIRD allocation
// of that slot. Entity numbers 0-511 are reserved; user slots start at 512.
//
// When engine.removeEntity(e) is called, DCL adds the entity's number+version
// to its "removed" set. The NEXT engine.addEntity() that needs a fresh slot
// returns (number, version+1) — a new entity ID for the same slot number.
//
// This means:
//   • After maze rebuild #1 → entity numbers ~512-1272, version 0 (IDs ≈ 512-1272)
//   • After maze rebuild #2 → same slots, version 1 (IDs ≈ 65536+512)
//   • After maze rebuild #3 → version 2 (IDs ≈ 131072+512 ≈ 132,000) ← normal!
//
// The teardown+rebuild pattern below is correct: engine.removeEntity correctly
// frees the slot back to DCL's allocator. `syncWall` (called during gameplay)
// uses the same REUSE pattern per the optimize-scene skill, reusing entities
// in-place. For maze rebuilds, teardown+rebuild is simpler and equally correct.
//
// The hash check avoids teardown when the same data arrives twice in one
// connection cycle (rebuildFromState + maze_rebuild, which is very common).

export function buildMaze(wallTypes: number[], wallStates: number[]) {
  // Fast path: skip rebuild if layout is identical to the previous call.
  // Most commonly fires when rebuildFromState() and maze_rebuild arrive with
  // the same template in the same connection cycle (player joins active match).
  // syncWall() invalidates this hash whenever a wall is structurally changed
  // during gameplay, so a same-template next-match rebuild correctly rebuilds.
  const newHash = String(_hashWalls(wallTypes, wallStates))
  if (newHash === _lastBuildMazeHash && wallEntities.size > 0) {
    return  // layout unchanged — entity tree is already up-to-date
  }
  _lastBuildMazeHash = newHash
...
// ── syncWall ──────────────────────────────────────────────────────────────────
//
// `instant` = true → used by buildMaze for a full layout refresh:
//   • skip door slide animations (snap to position)
//   • spawn new walls without the rise animation
//   • this path reuses existing wall entities when possible — no entity is
//     destroyed unless it actually needs to disappear (layout change), which
//     is the recommended DCL entity-pooling pattern from the optimize-scene skill.
//
// `instant` = false (default) → used by onStateChange for live wall updates
//   during gameplay (door open/close animations, wall destruction with rise).

export function syncWall(idx: number, newType: number, newState: number, instant = false) {
  ...
  if (!shouldExist && existing) {
    // Structural change (wall removed) — invalidate the buildMaze hash so that
    // a future maze_rebuild with the same template forces a proper rebuild and
    // restores walls that were destroyed during gameplay.
    _lastBuildMazeHash = ''
  ...
  if (shouldExist && !existing) {
    // Structural change (wall added during play or rebuild) — invalidate hash.
    _lastBuildMazeHash = ''
    spawnWallEntity(idx, newType, newState, instant)
    return
  }
  ...
  if (existing) {
    ...
    if (isDoor) {
      if (doorOpen && !wasDoorOpen) {
        ...
        // Door stayed closed (SOLID↔BLOCKED transitions, claim colour changes…).
        // Skip restoring the collider if a VIP-door-cross flash is active for
        // this door — flashVipDoorCross removed the collider and its delay
        // callback will restore it once the close-slide finishes. Restoring it
        // here would make the door physically solid while the mesh is still
        // animating open, blocking players who should see it as passable.
        if (!_flashingDoors.has(idx)) {
          MeshCollider.setBox(existing.entity, ColliderLayer.CL_PHYSICS | ColliderLayer.CL_POINTER)
...
export function syncWallHP(idx: number, type: number, hp: number, maxHp: number): void {
  ...
  if (!w) return
  // Open doors have no MeshCollider — skip to avoid "Missing MeshCollider" warning.
  if (w.state === STATE_OPEN) return
  const s      = getLocalState()
...
export function spawnBombOverlay(key: string, col: number, row: number, fuseEndsAt: number, armed = true) {
  ...
  if (!armed) {
    ...
    makeClickable(pickupHitbox, 'Pick up bomb', () => {
      // Same radius-1 rule: adjacent tile OK if no wall between us.
      const s = getLocalState()
      if (!isPickupInRange(s.myTileCol | 0, s.myTileRow | 0, col, row)) return
...
export function syncItems(
  ...
  for (const [key, item] of serverItems) {
    ...
    makeClickable(colliderEnt ?? ent, `Pick up ${item.type}`, () => {
      // Pickup allowed from the item's tile OR one cardinal step away,
      // provided no wall (including open doors) separates the two tiles.
      // Mirrors server canPickupBetweenTiles.
      const s = getLocalState()
      if (!isPickupInRange(s.myTileCol | 0, s.myTileRow | 0, item.tileCol, item.tileRow)) return
...
// Doors currently mid-flash. syncWall skips adding the collider for these
// because flashVipDoorCross already removed it and will re-add it in the
// delay callback. Without this guard, the patchRate state-update (which
// arrives ~100ms after the broadcast) would re-add the collider while the
// door mesh is still in the open-slide phase, making the door physically
// solid while it looks open.
const _flashingDoors = new Set<number>()
