...
export function updateVipClickable() {
  ...
  if (myTeam === 'hater') {
    // ALLOW_ALL_TILE_OVERLAP drops CL_PHYSICS so haters can walk straight
    // through the VIP. Click-target (CL_POINTER) stays so 'Attack VIP' works.
    const layer = s.allowAllTileOverlap
      ? ColliderLayer.CL_POINTER
      : (ColliderLayer.CL_POINTER | ColliderLayer.CL_PHYSICS)
    MeshCollider.setBox(vipHitBox, layer)
