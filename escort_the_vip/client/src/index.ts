...
import { setupZoneTriggers, isInsideGameZone, isInsideSpectatorZone }  from './zones/zoneTriggers'
import { buildMaze, createGameFloor, syncWall, syncItems, setMazeVisible, setSafetyTiles, syncWallHP, clearWallHPOverlays, spawnBombOverlay, removeBombOverlay, tickBombOverlays, clearAllBombOverlays, refreshJunctionsAroundWall, refreshDoorMaterials, flashVipDoorCross, syncStripeColor, refreshWallEKeyLabels, spawnExplosionEffect, anyAllyInSafetyZone, applySpectatorWallScale } from './game/maze'
...
import { upsertVip, setVipVisible, getVipEntity, updateVipClickable, setVipFacingYaw, refreshVipBombHint }  from './game/npcRenderer'
import { upsertHealthBar, removeHealthBar, showHealthBar } from './game/healthBar'
import { upsertEnemyHitbox, removeEnemyHitbox, syncEnemyHitboxTeams, syncAllEnemyHitboxesForOverlap, refreshEnemyBombHints } from './game/playerHitboxes'
...
import { GAME_FLOOR_Y, WALL_HEIGHT, tileToWorldX, tileToWorldZ, worldToTileCol, worldToTileRow } from './game/constants'

// ── Config ────────────────────────────────────────────────────────────────────

const SERVER_URL = 'wss://<your-domain>'
...
// Last tile ACTUALLY SENT to the server. Used by per-frame tile detection so
// a server-side teleport (which updates myTileCol/myTileRow immediately but
// moves the DCL avatar on the NEXT frame) does not trigger a false re-report
// of the old physical position. Seeded by the teleport message handler.
let _lastSentTileCol = -1
let _lastSentTileRow = -1

// VirtualCamera spectator state
const CAM_BACK   = 4.0   // horizontal offset behind watched target (metres)
const CAM_PITCH  = 40    // look-down angle (°) — calibrated for CAM_HEIGHT
// Camera Y: 2.5 m above wall tops (GAME_FLOOR_Y + WALL_HEIGHT = 17.5 m).
// The critical clearance condition: for a wall 1 m behind the player (closest
// possible wall edge, tile = 2 m) at 3 m from the camera, the line of sight
// must pass above wall tops (17.5 m).
//   y_at_wall = CAM_HEIGHT - (CAM_HEIGHT - player_y) * (b-1)/b
//   With CAM_HEIGHT=20, CAM_BACK=4, player_y≈16.8:
//   y_at_wall = 20 - 3.2 * (3/4) = 17.6 > 17.5  ✓
// Previously (CAM_HEIGHT=18.5, CAM_BACK=3): 18.5 - 1.7*(2/3) = 17.37 < 17.5 ✗
const CAM_HEIGHT = GAME_FLOOR_Y + WALL_HEIGHT + 2.5   // 20.0 m
// Smoothing: exponential decay so responsiveness is framerate-independent.
// Lower K = slower/smoother follow; position lags slightly behind the player
// for a cinematic feel; rotation catches up even more lazily to hide jitter.
const CAM_POS_K  = 4.0   // position lerp rate  (1/s)  — was 10.0
const CAM_ROT_K  = 2.5   // rotation slerp rate (1/s)  — was 7.0
let _camSnapNeeded      = true
let _camJustActivated   = false   // true only on the very first frame after vcam activation
let _lastSpectateTarget = ''
let _lastHiddenBarId    = ''
let _camSmoothX   = 0    // lerped camera world X
let _camSmoothZ   = 0    // lerped camera world Z
let _camSmoothYaw = 0    // slerped yaw (radians, wrap-safe)
...
export function main() {
  ...
  engine.addSystem((dt: number) => {
    ...
    // Per-frame tile detection — reports to server IMMEDIATELY on any tile change.
    // This is critical for attack/interact range checks: the server validates
    // areAdjacent() using the last known player tile, so the server must know
    // the current tile before a click is sent. Without this, reportTilePosition()
    // (timer-based, fires every 0.5s) could leave a 0–500ms window where the
    // server has a stale tile → attack/door/VIP click is silently rejected.
    //
    // Uses _lastSentTileCol/_lastSentTileRow (seeded by teleport handler) instead
    // of s.myTileCol/s.myTileRow so a server teleport — which updates myTileCol
    // optimistically BEFORE movePlayerTo physically moves the avatar — does not
    // cause a false re-report of the old physical position on the next frame.
    {
      const _pt = Transform.getOrNull(engine.PlayerEntity)
      if (_pt) {
        const _ls = getLocalState()
        if (_ls.zone === 'game') {
          const _c = worldToTileCol(_pt.position.x)
          const _r = worldToTileRow(_pt.position.z)
          if (_c !== _lastSentTileCol || _r !== _lastSentTileRow) {
            _lastSentTileCol = _c
            _lastSentTileRow = _r
            patchLocalState({ myTileCol: _c, myTileRow: _r })
            const _rm = getRoom()
            if (_rm) _rm.send('player_tile', { col: _c, row: _r })
          }
        }
      }
    }

    // Periodic heartbeat — re-sends current tile every 0.5s so the server
    // stays in sync after reconnects even if the player hasn't moved.
    posTimer += dt
  ...
  engine.addSystem(() => {
    ...
    // ── Spectator: hide world-space beacons ──────────────────────────────
    // The VIP cylinder and the safety-zone box sit at CAM_HEIGHT (≈18 m) and
    // completely block the spectator follow-cam view. Spectators already have
    // VIP HP and safety status in the 2D HUD, so we simply disable both
    // beacons while isSpectating and let the normal path restore them when
    // the viewer leaves spectator mode.
    if (ls.isSpectating) {
      updateVipLight({ active: false, col: 0, row: 0, hpFrac: 0, haterTilesNearby: 0 })
      updateSafetyBeacon({
        phase: 'lobby',   // 'lobby' keeps the beacon hidden regardless of timer
        timeRemaining: 0, myTeam: 'none', mySessionId: '', vipFollowerId: '', alliesInSafety: false,
      })
      // Keep end-game light (PointLight only, invisible mesh) and stripe colours —
      // neither of those obstructs the camera view.
  ...
  engine.addSystem(() => {
    ...
    if (desired === 'gameFrozen') {
      InputModifier.createOrReplace(engine.PlayerEntity, {
        mode: InputModifier.Mode.Standard({ disableAll: true })
      })
    } else if (desired === 'spectator') {
      InputModifier.createOrReplace(engine.PlayerEntity, {
        mode: InputModifier.Mode.Standard({ disableAll: true })
      })
  ...
  // ── Spectator zone evictor ───────────────────────────────────────────────
  // Mirror of the game-zone evictor: if the avatar is physically inside the
  // spectator trigger area but the player is NOT spectating (e.g. they
  // reloaded while spectating and the game ended in the meantime), push them
  // back to the lobby spawn after ~1 s. Fires continuously until the avatar
  // leaves, so it is tolerant of lag between teleport and physics.
  let _specEvictAccum = 0
  engine.addSystem((dt: number) => {
    const s = getLocalState()
    // Allow: actively spectating, or game still running (countdown / playing)
    const gameActive = s.phase === 'countdown' || s.phase === 'playing'
    if (s.isSpectating || gameActive || !isInsideSpectatorZone()) {
      _specEvictAccum = 0
      return
    }
    _specEvictAccum += dt
    if (_specEvictAccum < 1.0) return
    _specEvictAccum = 0
    movePlayerTo({ newRelativePosition: Vector3.create(8, 1.1, 4) })
    patchLocalState({ zone: 'lobby', isSpectating: false })
    console.log('[Lobby] Evicted from spectator zone (not spectating / no match)')
  })
  ...
  Transform.create(specCamEntity, { position: Vector3.create(16, CAM_HEIGHT, 16) })
  ...
  engine.addSystem((dt: number) => {
    ...
    if (!s.isSpectating) {
      if (vcamActive) {
        ...
        vcamActive = false
        applySpectatorWallScale(false)   // restore walls to 2.5 m for players
      }
      ...
      _camSnapNeeded = true; _camJustActivated = false; _lastSpectateTarget = ''
      return
    }
    ...
    if (targetId === '__vip__') {
      ...
      if (vipT) {
        targetX = vipT.position.x; targetZ = vipT.position.z; gotTarget = true
      } else {
        // Entity not yet loaded by DCL (avatar streaming delay on first view).
        // Use server tile position as fallback so the VirtualCamera activates
        // immediately near the game area — this triggers DCL to stream the entity.
        const vr = room.state.vip
        if (vr?.active) {
          targetX = tileToWorldX(vr.tileCol)
          targetZ = tileToWorldZ(vr.tileRow)
          gotTarget = true
        }
      }
    } else {
    ...
    // Pre-position + seed smoothed state before activating vcam.
    // This prevents the engine from interpolating from the entity's default
    // position to the real target on the first frame after activation.
    if (!vcamActive) {
      _camSmoothX      = targetX
      _camSmoothZ      = targetZ
      _camSmoothYaw    = 0
      _camJustActivated = true   // allow one-time snap to "behind player" position
      const ct = Transform.getMutable(specCamEntity)
      ct.position.x = targetX; ct.position.y = CAM_HEIGHT; ct.position.z = targetZ
      MainCamera.getMutable(engine.CameraEntity).virtualCameraEntity = specCamEntity
      vcamActive = true
      applySpectatorWallScale(true)    // shrink walls to 2.0 m for clearer top-down view
    }

    // Framerate-independent lerp/slerp factors derived from delta-time.
    // Formula: alpha = 1 - exp(-K * dt) gives the same perceived speed at any FPS.
    const posAlpha = 1 - Math.exp(-CAM_POS_K * dt)
    const rotAlpha = 1 - Math.exp(-CAM_ROT_K * dt)
    ...
    if (targetId === '__vip__') {
      ...
      const vipT = vipEnt !== null ? Transform.getOrNull(vipEnt) : null
      if (!vipT) {
        // Entity not yet streamed — hold camera on tile position so DCL can load it
        const vr = room.state.vip
        if (vr?.active) {
          const fx = tileToWorldX(vr.tileCol), fz = tileToWorldZ(vr.tileRow)
          _camSmoothX = fx; _camSmoothZ = fz
          const camTf = Transform.getMutable(specCamEntity)
          camTf.position.x = fx; camTf.position.y = CAM_HEIGHT; camTf.position.z = fz
          camTf.rotation   = Quaternion.fromEulerDegrees(CAM_PITCH, 0, 0)
          
        }
        return
      }

      // Forward direction from VIP quaternion (pure-Y rotation)
      ...
      // Target position: behind VIP, above all walls (no clipping)
      const wantX   = vipT.position.x - nfwdX * CAM_BACK
      const wantZ   = vipT.position.z - nfwdZ * CAM_BACK
      const wantYaw = Math.atan2(nfwdX, nfwdZ)

      if (_camSnapNeeded) {
        // Snap only on first vcam activation so the camera starts behind the target
        // rather than drifting from the entity default position.
        // On a target switch (_camJustActivated=false) we let the lerp handle the
        // transition — the camera glides smoothly from the old target to the new one.
        if (_camJustActivated) {
          _camSmoothX = wantX; _camSmoothZ = wantZ; _camSmoothYaw = wantYaw
        }
        _camJustActivated = false
        _camSnapNeeded    = false
      }
      // Always lerp — on first activation wantX === _camSmoothX so it's a no-op.
      _camSmoothX += (wantX - _camSmoothX) * posAlpha
      _camSmoothZ += (wantZ - _camSmoothZ) * posAlpha
      {
        let dy = wantYaw - _camSmoothYaw
        while (dy >  Math.PI) dy -= 2 * Math.PI
        while (dy < -Math.PI) dy += 2 * Math.PI
        _camSmoothYaw += dy * rotAlpha
      }

      camT.position.x = _camSmoothX
      camT.position.y = CAM_HEIGHT
      camT.position.z = _camSmoothZ
      camT.rotation   = Quaternion.fromEulerDegrees(CAM_PITCH, _camSmoothYaw * (180 / Math.PI), 0)

    } else {
      const p = room.state.players.get(targetId)
      if (!p || p.zone !== 'game') return

      // Find watched player avatar by userId (= DCL wallet address).
      // More reliable than AvatarShape.name which can diverge from Colyseus
      // displayName. PlayerIdentityData.address === server p.userId.
      let watchedAvatarEnt: Entity | null = null
      if (p.userId) {
        for (const [ent, ident] of engine.getEntitiesWith(PlayerIdentityData)) {
          if ((ident as { address: string }).address === p.userId) {
            watchedAvatarEnt = ent; break
          }
        }
      }
      const avatarT = watchedAvatarEnt !== null ? Transform.getOrNull(watchedAvatarEnt) : null
      const tx = avatarT ? avatarT.position.x : tileToWorldX(p.tileCol)
      const tz = avatarT ? avatarT.position.z : tileToWorldZ(p.tileRow)

      if (avatarT) {
        // ── PRIMARY path: avatar entity found ──────────────────────────────
        // Extract the player's forward direction from their avatar quaternion.
        // DCL syncs the avatar body Y-rotation to the player's camera yaw every
        // frame, so this gives the exact horizontal look direction without lag.
        const q = avatarT.rotation
        const fwdX = 2 * (q.w * q.y + q.x * q.z)
        const fwdZ = 1 - 2 * (q.y * q.y + q.x * q.x)
        const fLen = Math.sqrt(fwdX * fwdX + fwdZ * fwdZ)
        const nhX  = fLen > 0.001 ? fwdX / fLen : 0
        const nhZ  = fLen > 0.001 ? fwdZ / fLen : 1

        // Desired camera position: behind the watched player, 1 m above wall tops.
        // CAM_HEIGHT guarantees the viewpoint is never inside maze geometry,
        // so no geometry intersection check is needed.
        const wantX   = avatarT.position.x - nhX * CAM_BACK
        const wantZ   = avatarT.position.z - nhZ * CAM_BACK
        const wantYaw = Math.atan2(nhX, nhZ)

        if (_camSnapNeeded) {
          // Snap only on first activation; target switches glide smoothly via lerp.
          if (_camJustActivated) {
            _camSmoothX = wantX; _camSmoothZ = wantZ; _camSmoothYaw = wantYaw
          }
          _camJustActivated = false
        ...
        // ── FALLBACK: avatar not yet loaded / out of streaming range ──────
        // Orbit smoothly around the tile position; rotation tracks the target.
        if (_camSnapNeeded) {
          // Seed look-at offset only on first activation; target switches glide.
          if (_camJustActivated) {
            _camSmoothX   = tx
            _camSmoothZ   = tz + CAM_BACK   // initial offset so look-at has a direction
            _camSmoothYaw = 0
          }
          _camJustActivated = false
          _camSnapNeeded    = false
        ...
        {
          // Auto: rotate toward tile
          const dirX = tx - _camSmoothX
          const dirZ = tz - _camSmoothZ
        ...
        const wantFbX = tx - Math.sin(_camSmoothYaw) * CAM_BACK
        const wantFbZ = tz - Math.cos(_camSmoothYaw) * CAM_BACK
        _camSmoothX += (wantFbX - _camSmoothX) * posAlpha
        _camSmoothZ += (wantFbZ - _camSmoothZ) * posAlpha
...
export function onRoomJoined(room: ReturnType<typeof getRoom>) {
  ...
  room.onStateChange((state: any) => {
    ...
    if (me) {
      ...
      if (invKey !== _lastInventoryKey) {
        ...
        const hasBomb = (me.leftHand ?? 'none') === 'bomb'
        refreshEnemyBombHints(hasBomb)
        refreshVipBombHint(hasBomb)
      }
    }
  ...
  room.onMessage('teleport', (data: { x: number; y: number; z: number; cx?: number; cy?: number; cz?: number; spectating?: boolean }) => {
    ...
    if (data.y > 14) {
      ...
      // Seed _lastSentTileCol/_lastSentTileRow so the per-frame tile detector
      // treats this as already-sent. Without this, the 1-frame gap while
      // movePlayerTo physically moves the avatar would trigger a false re-report
      // of the old avatar position (which the server would then validate and
      // possibly reject or teleport-back).
      _lastSentTileCol = newCol
      _lastSentTileRow = newRow
    }
  })
  ...
  room.onMessage('maze_rebuild', (data: {
    ...
    { const s = getLocalState(); refreshWallEKeyLabels(s.myRightHand, s.myLeftHand); const hasBomb = s.myLeftHand === 'bomb'; refreshEnemyBombHints(hasBomb); refreshVipBombHint(hasBomb) }
  ...
  room.onMessage('scene_config', (data: {
    firstPerson: boolean; roofPhysics: boolean;
    allowAllTileOverlap?: boolean; bombFuseMs?: number
  }) => {
    applySceneConfig(data.firstPerson, data.roofPhysics)

    // Bomb fuse time — keep localState in sync so "How to Play" text reflects
    // the actual configured value rather than a hardcoded constant.
    if (typeof data.bombFuseMs === 'number' && data.bombFuseMs > 0) {
      patchLocalState({ bombFuseMs: data.bombFuseMs })
    }
...
function reportTilePosition() {
  ...
  // Always re-send current tile as a heartbeat so the server stays in sync
  // after reconnects. Per-frame detection (main system above) already handles
  // immediate sends on tile change — this is the periodic re-sync safety net.
  // Server deduplicates same-tile reports (newCol===tileCol guard in player_tile).
...
function rebuildFromState(room: any) {
  ...
  } else if (serverZone === 'spectator') {
    // Spectator reconnect: restore isSpectating immediately so the VirtualCamera
    // and spectator UI activate on this frame — don't wait for the server's
    // teleport message which may arrive one tick later.
    const specIds: string[] = [...room2.state.players.keys()].filter((sid: string) => {
      const pl = room2.state.players.get(sid)
      return pl?.zone === 'game' && pl?.isAlive && sid !== room2.sessionId
    })
    if (room2.state.vip?.active) specIds.unshift('__vip__')
    const curTarget = s.spectatedIds[s.spectateIndex]
    const newIdx    = specIds.indexOf(curTarget)
    patchLocalState({
      zone:          'spectator',
      isSpectating:  true,
