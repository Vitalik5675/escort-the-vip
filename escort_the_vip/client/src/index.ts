...
import { buildGameZone }                           from './zones/gameZone'
import { setupZoneTriggers, isInsideGameZone }  from './zones/zoneTriggers'
import { buildMaze, createGameFloor, syncWall, syncItems, setMazeVisible, setSafetyTiles, syncWallHP, clearWallHPOverlays, spawnBombOverlay, removeBombOverlay, tickBombOverlays, clearAllBombOverlays, refreshJunctionsAroundWall, refreshDoorMaterials, flashVipDoorCross } from './game/maze'
...
import { upsertVip, setVipVisible, getVipEntity, updateVipClickable }  from './game/npcRenderer'
import { upsertHealthBar, removeHealthBar, showHealthBar } from './game/healthBar'
import { upsertEnemyHitbox, removeEnemyHitbox, syncEnemyHitboxTeams } from './game/playerHitboxes'
import { triggerHitFlash }    from './game/screenEffects'
import { triggerPlayerHitHalo, removePlayerHalo, notifyVipHit, updateVipLight, updateEndGameLight, countNearbyHaters, resetAllLightEffects } from './game/lightEffects'
import { setupAllUI }         from './ui'
import { GAME_FLOOR_Y, tileToWorldX, tileToWorldZ, worldToTileCol, worldToTileRow } from './game/constants'

// ── Config ────────────────────────────────────────────────────────────────────

const SERVER_URL = 'wss://your-domain.duckdns.org'

const DISCONNECTED_TIMEOUT_MS = 8_000   // teleport to lobby after 8s offline

// ── Damage HUD ────────────────────────────────────────────────────────────────
// Window during which incoming/outgoing hits sum into a single number — matches
// server ATTACK_RESOLVE_DELAY_MS so a counter-attack swap arriving in the same
// tick is reported as one combined amount instead of two flashes.
const DAMAGE_WINDOW_MS = 700
// Total time the number stays visible after the last hit before fading.
const DAMAGE_VISIBLE_MS = 3_000

function accumulateDamage(kind: 'dealt' | 'recv', amount: number): void {
  if (!amount) return
  const now = Date.now()
  const s   = getLocalState()
  if (kind === 'dealt') {
    const within = now < s.dmgDealtUntilMs
    patchLocalState({
      dmgDealtAmount:  within ? s.dmgDealtAmount + amount : amount,
      dmgDealtUntilMs: now + DAMAGE_WINDOW_MS,
      dmgDealtShownMs: now + DAMAGE_VISIBLE_MS
    })
  } else {
    const within = now < s.dmgRecvUntilMs
    patchLocalState({
      dmgRecvAmount:  within ? s.dmgRecvAmount + amount : amount,
      dmgRecvUntilMs: now + DAMAGE_WINDOW_MS,
      dmgRecvShownMs: now + DAMAGE_VISIBLE_MS
    })
  }
}
...
// Last-seen lastDamageTime per session id and for the VIP — used to spawn the
// per-target on-hit halo exactly once per server-confirmed hit (state diff,
// not the local 'hit' message, so spectators also see it).
...
export function main() {
  ...
  engine.addSystem(() => {
    ...
    if (vip?.active) {
      ...
      const haters = countNearbyHaters(vip.tileCol, vip.tileRow, players as any)
  ...
  type ModState = 'none' | 'spectator' | 'gameJump'
  ...
  engine.addSystem(() => {
    const s = getLocalState()
    const shouldShow = s.phase === 'countdown' || s.phase === 'playing'
  ...
  engine.addSystem((dt: number) => {
    const s = getLocalState()
    const matchActive = s.phase === 'countdown' || s.phase === 'playing'
...
export function onRoomJoined(room: ReturnType<typeof getRoom>) {
  ...
  room.onStateChange((state: any) => {
    ...
    // Per-player hit halo: spawn whenever the server increments a player's
    // lastDamageTime. Detected here (not in the local 'hit' message handler)
    // so spectators following the victim also see the visual flash.
    const allPlayers = state.players
    if (allPlayers) {
      for (const [sid, p] of allPlayers as Iterable<[string, {
        lastDamageTime: number; lastDamage: number;
        tileCol: number; tileRow: number;
        zone: string; isAlive: boolean
      }]>) {
        const ldt = p.lastDamageTime ?? 0
        const prev = _prevPlayerHitTime.get(sid) ?? 0
        if (ldt > prev) {
          _prevPlayerHitTime.set(sid, ldt)
          if (p.zone === 'game' && p.isAlive && prev > 0) {
            triggerPlayerHitHalo(sid, p.tileCol, p.tileRow, p.lastDamage ?? 0)
          }
        }
      }
    }
    ...
    if (vip) {
      ...
      const vipKey = `${vip.active},${vip.tileCol},${vip.tileRow},${vip.female},${vip.outfitIndex},${vip.skinIndex},${vip.hairIndex},${vip.eyeIndex}`
    ...
    if (!stateReady) {
      ...
      state.bombs?.onAdd?.((b: any, key: string) => {
        spawnBombOverlay(key, b.tileCol, b.tileRow, b.fuseEndsAt)
        patchLocalState({
          activeBombs: [...getLocalState().activeBombs, { key, col: b.tileCol, row: b.tileRow, fuseEndsAt: b.fuseEndsAt, armed: !!b.armed, triggerWallIdx: b.triggerWallIdx | 0 }]
        })
      })
  ...
  room.onMessage('vip_room_unlocked', () => {
    console.log('[Scene] VIP room unlocked')
  })
  ...
  room.onMessage('hit', (data: { damage: number; attackerId?: string; shield?: number; blocked?: boolean }) => {
    triggerHitFlash()
    patchLocalState({ lastHitTimeMs: Date.now() })
    accumulateDamage('recv', data?.damage ?? 0)
  })

  // Server tells the ATTACKER how much HP damage their swing dealt to the target.
  room.onMessage('hit_dealt', (data: { damage: number; targetId?: string }) => {
    accumulateDamage('dealt', data?.damage ?? 0)
  })
  ...
  room.onMessage('bomb_explode', (_data: { col: number; row: number }) => {
...
export function onRoomLeft() {
  ...
  // Clear lingering halos / VIP light / end-game pulse — the per-frame system
  // will rebuild them once the next match starts.
